...
corrects is the EVIDENCE several passages above rest on, and it names the
passages so that a reader checking one of them looks in the right place.

**What was wrong with the evidence.** Each passage named below states what the
reference does at a point where no case in the vendored corpus reaches, and
each rested either on a reading of the reference's source, or on a run against
a checkout three commits past the tag this package vendors its corpus from.
Neither is the tag. The reference transcript is the apparatus that fixes this:
`conformance/transcript/transcript.json` holds rows generated by running the
reference at the tag, with the tag, the commit, the corpus hash, the toolchain
and a hash of the file itself recorded beside them in
`conformance/transcript/SOURCE.json`, and `test/reference-transcript.test.ts`
diffs this package's answer against every row, declaring each row where the two
differ with both answers so that the row fails when either side moves.

### The passages whose reference half is now a row

**The float spelling, in the amendment headed "one spelling of a float,
negative zero included".** Its closing section says the digits are the host's
and reports the renderings `1.0e20`, `1.0e21` and `1.0e-6` from a run made for
that amendment. Those three renderings are rows: `float-cast/1e20`,
`float-cast/1e21` and `float-cast/1e-6`, each with a `float-json/` twin, and
each is declared in `test/reference-transcript.test.ts` with this package's
spelling beside the reference's. Read that section's run as superseded by
those rows as evidence; what it states is unchanged.

**An arithmetic result past the safe integer range, in the Consequences
section, and again in the closing paragraph of the amendment headed "a `lit`
operand refuses an integer outside the safe range".** Both say the reference's
integers are arbitrary precision and its sum of two large integers is exact.
Asked at the tag, it answered the exact sum and the exact product where this
package refuses with `"integer_out_of_range"`. Those are the rows
`integer-range/sum-past-safe` and `integer-range/product-past-safe`; the row
`integer-range/sum-at-bound` beside them agrees on both sides. Each asks for
its result as text, because a row whose answer is an integer outside the safe
range is one this package's own decoder refuses, which is a limit of the
transcript rather than of the reference. The refusal here is `numericResult` in
`src/evaluator.ts`, read at `6aa2d92`.

**An integer key against a map, in the amendment headed "an integer bracket key
reads its string spelling".** Its second declared divergence - a map holding
only a string-spelled numeric key is reachable by an integer key here, where
the reference answers the absence - is now the row
`map-key/integer-against-string-spelling`. Asked at the tag for the integer key
against a map holding that key's string spelling, the reference answered the
absence; this package answers the occupant. The lookup here is `bracketAccess`
in `src/evaluator.ts`, read at `6aa2d92`.

**A value with no JSON form, declared at `serialize` in
`src/functions/json.ts`.** That declaration said the reference falls back to its
host language's own inspect rendering for a value its serializer cannot take.
**That is inexact, and the correction is the reason this passage is here rather
than in the section above.** Asked at the tag, the reference's encoder took all
four and wrote a JSON value for each: a date as the JSON string
`"2026-09-19"`, an instant as the JSON string
`"2026-09-19T10:30:00.000000Z"` - the encoder writes the microseconds, and the
row carries them - a
duration as a JSON object of its parts, and the absence as the JSON string
`"undefined"`. That last is the encoder's own JSON form for the absence, not a
rendering of it - the host language renders the same value as `:undefined` -
so the fallback the declaration named runs for none of the four. Those are the
rows whose ids begin `json-form/`. The rule the declaration states, that this
package refuses all four, is unchanged, and the declaration itself is corrected
in the same change as this note: it now says that the encoder took all four.

**A leading sign on a date or datetime text, declared at `readDate` and at
`DATETIME_TEXT` in `src/iso.ts`.** The reference reads such a sign as the sign
of the year and this package refuses either sign. The plus is the row:
`leading-sign/date-plus` and `leading-sign/datetime-plus` hold the date and the
instant the reference answers where this package answers the absence.

### The declarations no row can carry, and why

**A minus before a date or datetime text.** The reference answers a negative
year for `-2026-09-19` and the year zero for `-0000-01-01`. The wire form a
transcript row is read back through writes a year as four unsigned digits and
holds only years from one hundred up, by its own condition at `isWireDate` in
`src/tagged.ts`, read at `6aa2d92`. So a row carrying either answer stops the
whole file from being read. The plus rows put the same refusal to the
reference, and the refusal of a minus stays pinned on the side that can be
executed.

**A map with a boolean-keyed hit, in the Consequences section.** The reason
that section already gives is the reason a row cannot carry it: a row's context
is JSON-shaped, so its map keys are strings, and the tagged encoding has no
boolean-keyed map form. The row `map-key/boolean-against-string-spelling` asks
a boolean key of a map holding the TEXT of that boolean, which both sides
answer as a miss, so it agrees and is not declared; it bounds the gap rather
than closing it. That paragraph's claim about the reference stands on this
record's existing evidence and is not converted here.

**What a write under an integer key leaves behind** - the first and third
declared divergences of the integer-key amendment, and every reference-side
sentence of the note headed "statement mode, the two program entry points, and
the store write". A transcript row runs an expression, and a write needs the
statement entry point, which the transcript's generator does not reach. Those
sentences still rest on runs against a checkout past the tag, as that note
already says of itself, and converting them needs an apparatus that runs a
program rather than an expression.

### The one row whose answer is a property of the run

**The memoized clock, declared at `clockFunction` in `src/functions/date.ts`,
read at `6aa2d92`.** This package reads the host clock at most once per
evaluation, so two calls inside one evaluation answer one instant; the
reference reads its own on every call. Asked at the tag whether two reads
inside one expression answer the same instant, it answered that they do not,
and that is the row `clock/two-reads-in-one-evaluation`: false there, true
here. Unlike every other row, its answer follows from two clock reads landing
in different microseconds rather than from the tag alone. A regeneration that
answered otherwise is a coincidence to re-run before it is read as a change in
the reference, and the transcript's generator says so where the case is
authored. The declaration itself is unchanged: it remains a decision rather
than a discovery.

### What this note does not decide

It amends no rule, adds no obligation, and changes no reason token. It does not
