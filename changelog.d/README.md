...
- a change in what the evaluator accepts or refuses
- anything breaking
...
- The evaluator answers a comparison between a date and a duration.
```
...
- A change in what the conformance corpus expects, or in the ISA version this
  build implements, is visible to every consumer holding a compiled instruction
  list. Say what to change on the consumer's side.
